...
let GitHubViewer = React.createClass({
    ...
    transformHtml: (description) => {
        ...
        for (let word of description.split(' ')) {
            if (userSet.has(word)) {
                if (paragraph.length) {
                    list.push(<span key={`element${list.length}`}>{paragraph}&nbsp;</span>);
                }
                list.push(<a key={`element${list.length}`}
                             href={`https://github.com/${word.replace('@', '')}`}>{word}</a>)
                paragraph = '';
            } else {
    ...
    render() {
        ...
        if (this.props.view === 'list') {
            view = <IssuesList {...this.props} transformHtml={this.transformHtml}/>
        } else {
            view = <IssueDetails {...this.props} onClick={this.props.showList} transformHtml={this.transformHtml}/>
        }
        ...
        return (
            <div className='index'>
                <div className="header">
                    ...
                    <svg aria-hidden="true" class="octicon octicon-mark-github" height="28" version="1.1"
                         viewBox="0 0 16 16" width="28">
                        <path
